...
const bilinearInterpolate = (sourceCanvas, scale) => {
  /**
   * Creates an ImageData object from a given canvas
   *
   * @param {canvas} canvas - source canvas
   * @return {ImageData} ImageData obtained from canvas
   */
  ...
  const sourceImageData = createImageData(sourceCanvas);
  const destinationImageData = createImageData(scaledCanvas);
